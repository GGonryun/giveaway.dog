...
const generateMockUser = (index: number): User => ({
  ...
  lastEntryAt: new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000).toISOString(),
  firstEntryAt: new Date(Date.now() - Math.random() * 90 * 24 * 60 * 60 * 1000).toISOString(),
  ...
  tags: index % 3 === 0 ? ['VIP', 'High Value'] : index % 5 === 0 ? ['Suspicious'] : []
});
