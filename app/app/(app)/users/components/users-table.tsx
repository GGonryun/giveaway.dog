...
export const UsersTable = ({ initialPage = DEFAULT_PAGE, initialPageSize = DEFAULT_PAGE_SIZE }: UsersTableProps) => {
  ...
  const [sortDirection, setSortDirection] = useState<'asc' | 'desc'>(() => {
    return (searchParams.get('sortDirection') as 'asc' | 'desc') || DEFAULT_SORT_DIRECTION;
  });
  const [filters, setFilters] = useState(() => ({
    query: searchParams.get('search') || DEFAULT_FILTERS.query,
    minScore: searchParams.get('minScore') ? parseInt(searchParams.get('minScore')!, 10) : DEFAULT_FILTERS.minScore,
    maxScore: searchParams.get('maxScore') ? parseInt(searchParams.get('maxScore')!, 10) : DEFAULT_FILTERS.maxScore,
  ...
  const loadUsers = useCallback(async (searchQuery?: string, immediate = false) => {
    ...
    const actualQuery = searchQuery !== undefined ? searchQuery : filters.query;
  ...
  }, [currentPage, pageSize, filters.query, sortField, sortDirection, filters.status]);
  ...
  }, [currentPage, pageSize, sortField, sortDirection, filters.status, filters.source, filters.minScore, filters.maxScore, filters.dateRange]);
  ...
  const getStatusBadge = (status: string) => {
    const variants = {
      active: { variant: 'default' as const, label: 'Active', icon: CheckCircle, className: 'bg-blue-500 hover:bg-blue-600 text-white' },
      flagged: { variant: 'destructive' as const, label: 'Flagged', icon: AlertTriangle, className: '' },
      blocked: { variant: 'secondary' as const, label: 'Blocked', icon: UserX, className: '' },
      trusted: { variant: 'default' as const, label: 'Trusted', icon: UserCheck, className: '' }
    };
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  ...
                  <Badge variant="secondary">{totalUsers.toLocaleString()} total</Badge>
                </div>
              </div>
              <CardDescription>
                Manage and analyze your user base with comprehensive filtering and bulk operations.
              </CardDescription>
              ...
              <div className="flex flex-col space-y-4 lg:flex-row lg:items-center lg:justify-between lg:space-y-0 lg:space-x-4 pt-4">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <Button variant="secondary" size="sm" onClick={() => setShowCRMSheet(true)}>
            ...
            <CardContent className="p-0">
                      ...
                      >
                            ...
                            <div className="w-16 bg-muted rounded-full h-1.5">
                              <div 
                                className={`h-1.5 rounded-full transition-all ${
                                  user.engagement >= 80 ? 'bg-green-500' : 
                                  user.engagement >= 60 ? 'bg-blue-500' : 
                                  user.engagement >= 40 ? 'bg-yellow-500' : 'bg-red-500'
                                }`}
                                style={{ width: `${user.engagement}%` }}
                              />
                            </div>
                            <span className="text-xs font-medium min-w-[2.5rem]">{user.engagement}%</span>
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="text-sm">{formatDate(user.lastEntryAt)}</div>
                        </TableCell>
                        <TableCell>
                          <div className="flex items-center space-x-2">
                            <div className="w-16 bg-muted rounded-full h-1.5">
                            ...
                            <span className="text-xs font-medium min-w-[2rem]">{user.qualityScore}</span>
                          </div>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell onClick={(e) => e.stopPropagation()}>
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" side="bottom" sideOffset={4} avoidCollisions={true}>
              ...
              {!loading && (
                ...
                <div className="flex items-center space-x-2 text-sm text-muted-foreground">
                  <span>
                    Showing {((currentPage - 1) * pageSize) + 1} to {Math.min(currentPage * pageSize, totalUsers)} of {totalUsers.toLocaleString()} users
                  </span>
                </div>
