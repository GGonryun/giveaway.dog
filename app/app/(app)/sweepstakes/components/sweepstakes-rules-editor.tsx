...
export function SweepstakesRulesEditor() {
  ...
  const [customBlocklist, setCustomBlocklist] = useState('spam@example.com\nbadactor@fake.com');
        ...
        <div className="space-y-6">
            ...
            <div className="grid gap-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Automatically flag suspicious behavior patterns</p>
                </div>
                <Switch
                  checked={globalSettings.enableBotDetection}
                  onCheckedChange={(checked) => 
                    setGlobalSettings(prev => ({ ...prev, enableBotDetection: checked }))
                  }
                />
              </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Users must verify email to qualify</p>
                </div>
                <Switch
              ...
              <div className="flex items-center justify-between">
                <div>
                  <Label className="text-sm font-medium">Geographic Tracking</Label>
                  <p className="text-xs text-muted-foreground">Track user locations for analytics</p>
                </div>
                <Switch
                  checked={globalSettings.enableGeoTracking}
                  onCheckedChange={(checked) => 
                    setGlobalSettings(prev => ({ ...prev, enableGeoTracking: checked }))
                  }
                />
              </div>
            </div>
                ...
                <div key={rule.id} className="border rounded-lg p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="space-y-2">
                      ...
                      <Select
                        value={rule.type}
                        onValueChange={(type: Rule['type']) => updateRule(rule.id, { type })}
                      >
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      placeholder={
                        rule.type === 'rate_limit' ? 'e.g., 5' :
                        rule.type === 'geo_block' ? 'e.g., CN,RU,KP' :
                        rule.type === 'email_domain' ? 'e.g., tempmail.com,fake.com' :
                        'e.g., 192.168.1.1'
                      }
                      className="h-8"
                    />
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center space-x-4">
                ...
                <span className="text-sm text-muted-foreground">
                  Entries with risk scores above this threshold will be flagged for review
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="text-sm">
              <div className="font-medium text-blue-800 mb-1">Active Protection</div>
              <div className="text-blue-600 space-y-1">
                <div>• {rules.filter(r => r.enabled).length} custom rules enabled</div>
                <div>• Bot detection: {globalSettings.enableBotDetection ? 'On' : 'Off'}</div>
                <div>• Email verification: {globalSettings.requireEmailVerification ? 'Required' : 'Optional'}</div>
                <div>• Max entries per user: {globalSettings.maxEntriesPerUser}</div>
              </div>
            </div>
          </div>
        </div>
