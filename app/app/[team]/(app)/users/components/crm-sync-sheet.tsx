        ...
        prev.map((job) =>
          job.id === newJob.id
            ? {
                ...
                status: 'running',
                progress: 15,
                recordsProcessed: Math.floor(newJob.recordsTotal * 0.15)
              }
            : job
        )
      );
        ...
        prev.map((job) =>
          job.id === newJob.id
            ? {
                ...
                errors: [
                  'Rate limit warning: 3 requests throttled',
                  'Invalid phone number for user_789'
                ]
              }
            : job
        )
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      <h3 className="font-medium">
                        Connected to {crmStatus.provider}
                      </h3>
                      <Badge variant="default" className="text-xs">
                        Active
                      </Badge>
                  ...
                  <div className="text-sm text-muted-foreground">
                    ...
                    {crmStatus.lastSyncAt
                      ? formatDate(crmStatus.lastSyncAt)
                      : 'Never'}
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span>API Usage</span>
                    <span>
                      {crmStatus.apiCalls.toLocaleString()} /{' '}
                      {crmStatus.apiLimit.toLocaleString()}
                    </span>
                  </div>
                  <Progress value={apiUsagePercentage} className="h-2" />
                  <div className="text-xs text-muted-foreground">
                    {(100 - apiUsagePercentage).toFixed(1)}% remaining this
                    month
                  </div>
                </div>
                    ...
                    {selectedUsers.length > 0 ? (
                      ...
                      <div className="flex items-center justify-between p-3 border rounded-lg">
                        <div className="flex items-center space-x-2">
                          ...
                          <span>
                            Sync Pending Users ({crmStatus.pendingSync})
                          </span>
                        </div>
                  ...
                  <div key={job.id} className="border rounded-lg p-4">
                    ...
                    {job.status === 'running' && (
                      <div className="space-y-2 mb-3">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {job.progress}% complete • Running for{' '}
